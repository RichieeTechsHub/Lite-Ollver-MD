  ...
  try {
    ...
    if (!quotedInfo?.quotedMessage) {
      return sock.sendMessage(msg.key.remoteJid, {
        text: "❌ Reply to an audio/video message with .reverse"
      });
    }
    ...
    const input = path.join(tempDir, "reverse-input-" + Date.now());
    const output = path.join(tempDir, "reverse-output-" + Date.now() + ".mp3");

    await fs.writeFile(input, buffer);

    const options = [];
    options.push({ type: "audioFilter", value: "areverse" });
    ...
    await sock.sendMessage(msg.key.remoteJid, {
      text: "❌ reverse failed. Reply to a valid audio/video file."
    });
  }
}
